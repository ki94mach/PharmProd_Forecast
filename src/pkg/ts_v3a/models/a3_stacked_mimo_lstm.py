"""V3A A3: stacked_mimo_lstm.

Two-layer LSTM + Dense(horizon), trained DIRECT/MIMO via :class:`NeuralTrainer`.
Same fit/predict/scaler contract as A2; only the Keras graph differs.
"""
...
def build_stacked_mimo_lstm(config: NeuralExperimentConfig) -> Any:
    """Fresh Keras graph: stacked LSTM (return_sequences) → Dense(horizon)."""
    from keras import Sequential
    from keras.layers import Dense, Input, LSTM

    lookback = int(config.lookback)
    units = int(config.hidden_units)
    units2 = int(config.second_hidden_units)
    ...
    return Sequential(
        ...
        name="stacked_mimo_lstm",
    )


def default_a3_config(**overrides: Any) -> NeuralExperimentConfig:
    """Config defaults for A3 — same training hyperparams as A2, stacked graph."""
    base = {
        "architecture_name": ArchitectureName.A3_STACKED_MIMO_LSTM.value,
        "lookback": 12,
        "hidden_units": 32,
...
class StackedMimoLSTM(BaseNeuralForecastModel):
    """A3 stacked MIMO LSTM wrapper around the shared :class:`NeuralTrainer`."""

    name: str = ArchitectureName.A3_STACKED_MIMO_LSTM.value

    def __init__(self, config: Optional[NeuralExperimentConfig] = None) -> None:
        cfg = config or default_a3_config()
        if cfg.architecture_name != ArchitectureName.A3_STACKED_MIMO_LSTM.value:
            cfg = cfg.with_architecture(ArchitectureName.A3_STACKED_MIMO_LSTM)
        super().__init__(cfg)
        ...
        self._architecture_builder = build_stacked_mimo_lstm
    ...
    def predict(self, window: ForecastWindow) -> ForecastResult:
        ...
        if self._trainer is None or self._trainer.model_ is None:
            raise RuntimeError("StackedMimoLSTM.predict called before fit")
        if self._scaler is None or self._last_lookback_raw is None:
            raise RuntimeError("StackedMimoLSTM missing scaler or lookback state")
        ...
        if scaled_preds.shape[0] != horizon:
            raise RuntimeError(
                f"stacked MIMO predict returned {scaled_preds.shape[0]} values; "
                f"expected {horizon}"
            )
